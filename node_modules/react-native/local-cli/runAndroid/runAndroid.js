...
function checkAndroid(args) {
  return fs.existsSync(path.join(args.root, '../MyApp/gradlew'));
}
...
function run(args, reject) {
  process.chdir(path.join(args.root, '../MyApp'));
  try {
    tryRunAdbReverse();
  ...
  try {
    ...
    if (devices && devices.length > 0) {
      devices.forEach((device) => {
        ...
        const unInstallArgs = [ 'uninstall', 'com.gaoxh.myapp'];

      console.log(chalk.bold(
          `unInstall the app on ${device} (${adbPath} ${unInstallArgs.join(' ')})...`
      ));

      child_process.spawnSync(adbPath, unInstallArgs, {stdio: 'inherit'});

        const installArgs = [ 'install', '../MyApp/app/build/outputs/apk/app-debug.apk'];

      console.log(chalk.bold(
          `install the app on ${device} (${adbPath} ${installArgs.join(' ')})...`
      ));

      child_process.spawnSync(adbPath, installArgs, {stdio: 'inherit'});

        const adbArgs = ['-s', device, 'shell', 'am', 'start', '-n', packageName + '/com.gaoxh.myapp.main.StartActivity'];
      ...
      const fallbackAdbArgs = [
        'shell', 'am', 'start', '-n', packageName + '/.main.MainReactActivity'
      ];
      console.log(chalk.bold(
